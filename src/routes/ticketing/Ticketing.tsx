...
function Ticketing() {
   const [selectedLabel, setSelectedLabel] = useState(EVENTID["1일차"]);
         ...
         openModal({
            ...
            body: (
               <div>
                  ...
                  }일차) 단국존 티켓 신청이 성공적으로 완료되었습니다. 내부적으로 검토 이후 선착순 1200명에 선정되어 티켓팅 성공한 경우 1~2일 내로 에 티켓팅 승인 결과가 등록된 휴대전화번호의 SMS로 전송됩니다.`}
                  <br />
                  <br />
   ...
   const canTicketing = (eventId: EVENTID) => {
      const now = new Date();

      const ticketingTime = new Date(
         eventId === EVENTID["1일차"]
            ? (process.env.REACT_APP_DAY_1_TICKETING_TIME as unknown as string)
            : (process.env.REACT_APP_DAY_2_TICKETING_TIME as unknown as string)
      );

      return now > ticketingTime;
   };

   const eventIdToReadableString = (eventId: EVENTID) =>
      eventId === EVENTID["1일차"] ? "17일" : "18일";

   return (
      <TicketingComponent.Container>
         <Tab
            labels={["17일 티켓", "18일 티켓"]}
            selectedLabel={eventIdToReadableString(selectedLabel) + " 티켓"}
            onLabelClick={(label) => {
               setSelectedLabel(
         ...
         <TicketingComponent.TicketThumbnail
            src={selectedLabel === EVENTID["1일차"] ? Thumbnail17 : Thumbnail18}
            alt={`${eventIdToReadableString(selectedLabel)} 티켓`}
         />

         {canTicketing(selectedLabel) ? (
            <TicketingComponent.Body>
               <div>
                  <h6>주의사항</h6>
                  <TicketingComponent.P>
                     * 단국대학교 죽전캠퍼스 재학생(졸업생, 휴학생 불가)만
                     신청할 수 있습니다.
                     <br />* 이름, 학번, 학과, 전화번호를 홈페이지 가입시
                     전화번호를 잘못 입력한 경우 이벤트 참여에 불이익이 발생할
                     수 있습니다.
               ...
               <TicketingComponent.Terms>
                  ...
                  DANFESTA 波:浪&gt;의 공식 예매처(단국대학교 홈페이지)가 아닌
                  다른 경로로 구매/취득한 티켓 중에서 매크로 등의 부정 한
                  방법으로 예매하거나 프리미엄 거래 사이트 및 개인 SNS 등에서
                  매매되는 티켓 및 팔찌를 모두 부정 거래로 간주하고, 이에 대해
                  엄격히 대처하고자 합니다.
                  <br /> 따라서, 아래와 같이 부정 거래 방침에 대해 사전 안내
                  드리오니 추후 불이익을 당하지 않도록 학우 여러분들의 유의와
                  양해를 부탁드립니다. <br />본 공연은 예매자 본인이 직접
                  예매하셔야 하며, 대리 예매 시도 및 양도 등으로 불이익 이
                  발생할 수 있습니다.
                  <br />
                  비정상적이거나 부정한 방법 (예: 매크로 사용 등)을 이용하거나,
                  안내된 유의사항을 지키지 않아 발생하는 모든 문제에 대해
                  총학생회, 총학생회 홈페이지 관리자는 일절 책임이 없으며,
                  해결의 주체가 될 수 없음을 알려드리오니 이와 관련한 불 이익을
                  당하지 않도록 각별히 주의하시기 바랍니다. <br />
                  <br />
                  해당 사유로 발생한 부정 거래는 사전 통보 없이 예매 취소 및
                  현장에서 사용 제한됩니다. <br />
                  <br />① 프리미엄 거래 사이트 및 개인 SNS 등에서 부정 티켓 및
                  팔찌 거래가 확인되는 경우, 티켓의 정보를 확인한 후 해당 티켓에
                  대한 예매를 취소할 예정입니다. 또한, 해당 예매자는
                  총학생회에서 주최하는 모든 행사의 참여 및 상품 수령이 제한될
                  예정입니다.
                  <br />
                  <br />② 매크로, 프리미엄 대리 예매 등을 이용한 부정한 방법으로
                  거래되는 행위를 예방하고자 총학생회는 총학생회 홈페이지
                  관리자와 긴밀한 협의를 하고 있습니다. 부정 예매로 인한
                  불이익을 당하지 않도록 학우 여러분들의 사전 유의 부탁드립니다.
                  <br />
                  ...
                  유의하시길 바랍니다. <br />
                  <br />
                  어떠한 이유를 막론하고 부정 예매, 부정 거래로 간주되는 건의
                  해당 티켓은 사전 안내 없이 취소됩니다. 또한 당일 팔찌를
                  소지하고 있더라도 입장이 제한될 수 있습 니다.
               </TicketingComponent.Terms>
